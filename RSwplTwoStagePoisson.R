...
fit_WPL<-function(y,g,x, pos, sc,fss,  n2infor, N2,  pars){
   ...
   gr<-function(theta){
      wincrementda=wdalpha(y[i],y[j],g[i],g[j],x[i],x[j], alpha=theta[1],beta=theta[2],
                           sigma2=exp(theta[3]),tau2_11=exp(theta[4]), tau_12=theta[5],
                           tau2_22=exp(theta[6]), pos[i], pos[j], sc[i], sc[j], 
                           fss, n2infor,N2)
      wincrementdb=wdbeta(y[i],y[j],g[i],g[j],x[i],x[j],alpha=theta[1],beta=theta[2],
                          sigma2=exp(theta[3]),tau2_11=exp(theta[4]),tau_12=theta[5], tau2_22=exp(theta[6]), pos[i], pos[j], sc[i], sc[j],
                          fss, n2infor,N2)
      wincrementds=exp(theta[3])*wdsigma2(y[i],y[j],g[i],g[j],x[i],x[j],
                                          alpha=theta[1],beta=theta[2],sigma2=exp(theta[3]),tau2_11=exp(theta[4]),tau_12=theta[5], tau2_22=exp(theta[6]), pos[i], pos[j], sc[i], sc[j], 
                                          fss, n2infor,N2)
